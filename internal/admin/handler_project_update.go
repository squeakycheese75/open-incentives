...
func (s *Handler) UpdateProject(w http.ResponseWriter, r *http.Request) {
	...
	out, err := s.adminContainer.UpdateProjectUsecase(authCtx.OrgID).Execute(r.Context(), domain.UpdateProjectUsecaseInput{
	...
	if err != nil {
		...
		default:
			httputil.WriteJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed_to_update_project"})
		}
		return
	}

	httputil.WriteJSON(w, http.StatusOK, projectToResponse(out.Project))
}
